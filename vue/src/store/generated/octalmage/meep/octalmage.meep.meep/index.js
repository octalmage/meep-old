    ...
    actions: {
        ...
        async QueryTipAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await queryClient.queryTipAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
        ...
        async QueryUsernameAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await queryClient.queryUsernameAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    value = mergeResults(value, next_values);
                }
        ...
        async QueryThreadAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await queryClient.queryThreadAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    value = mergeResults(value, next_values);
                }
        ...
        async sendMsgSubmitBanUserProposal({ rootGetters }, { value, fee = [], memo = '' }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgSubmitBanUserProposal(value);
                const result = await txClient.signAndBroadcast([msg], { fee: { amount: fee,
                        gas: "200000" }, memo });
                return result;
            }
                ...
                if (e == MissingWalletError) {
                    throw new SpVuexError('TxClient:MsgSubmitBanUserProposal:Init', 'Could not initialize signing client. Wallet is required.');
                }
                else {
                    throw new SpVuexError('TxClient:MsgSubmitBanUserProposal:Send', 'Could not broadcast Tx: ' + e.message);
                }
            }
        },
        ...
        async MsgSubmitBanUserProposal({ rootGetters }, { value }) {
            try {
                const txClient = await initTxClient(rootGetters);
                const msg = await txClient.msgSubmitBanUserProposal(value);
                return msg;
                ...
                if (e == MissingWalletError) {
                    ...
                    throw new SpVuexError('TxClient:MsgSubmitBanUserProposal:Create', 'Could not create message: ' + e.message);
                }
            }
        },
